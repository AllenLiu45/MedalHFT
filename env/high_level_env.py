...
tech_indicator_list = np.load('./MyData/feature_list/single_features.npy', allow_pickle=True).tolist()
tech_indicator_list_trend = np.load('./MyData/feature_list/trend_features.npy', allow_pickle=True).tolist()
minute1_indicator_list = np.load('./MyData/feature_list/minute1_features.npy', allow_pickle=True).tolist()
minute3_indicator_list = np.load('./MyData/feature_list/minute3_features.npy', allow_pickle=True).tolist()
minute5_indicator_list = np.load('./MyData/feature_list/minute5_features.npy', allow_pickle=True).tolist()
clf_list = ['slope_360', 'vol_360', 'liq_360']
...
class Testing_Env(gym.Env):
    def __init__(
        ...
        clf_list=clf_list,
        minute1_indicator_list=minute1_indicator_list,
        minute3_indicator_list=minute3_indicator_list,
        minute5_indicator_list=minute5_indicator_list,
        ...
        self.data_window = self.df.iloc[self.m - self.stack_length:self.m + self.look_back_window - 1]
        self.data = self.df.iloc[self.m - self.stack_length + self.look_back_window - 1: self.m + self.look_back_window - 1]
        ...
        self.minute1_state = self.data_window[minute1_indicator_list].values
        self.minute3_state = self.data_window[minute3_indicator_list].values
        self.minute5_state = self.data_window[minute5_indicator_list].values
    ...
    def reset(self):
        ...
        if (self.m - self.stack_length < 0 or
                self.m + self.look_back_window - 1 > len(self.df) or
                self.m - self.stack_length + self.look_back_window - 1 > len(self.df)):
            self.terminal = True

        self.data_window = self.df.iloc[self.m - self.stack_length:self.m + self.look_back_window - 1]
        self.data = self.df.iloc[
                    self.m - self.stack_length + self.look_back_window - 1: self.m + self.look_back_window - 1]


        if self.data.empty or self.data_window.empty or self.terminal == True:
            self.terminal = True

            return np.array([]), np.array([]), np.array([]), np.array([]), np.array([]), self.terminal, {
        ...
        return self.single_state, self.trend_state, self.clf_state.reshape(-1), self.minute1_state, self.minute3_state, self.minute5_state, self.terminal, {
            "previous_action": self.initial_action,
        }
    ...
    def step(self, action):
        ...
        self.terminal = (self.m >= len(self.df) - self.look_back_window)
        ...
        return self.single_state, self.trend_state, self.clf_state.reshape(-1), self.minute1_state, self.minute3_state, self.minute5_state, self.reward, self.terminal, {
...
class Training_Env(Testing_Env):
    def __init__(
        ...
        super(Training_Env,
              self).__init__(df, tech_indicator_list, tech_indicator_list_trend, clf_list, minute1_indicator_list, minute3_indicator_list, minute5_indicator_list, transcation_cost,
                             back_time_length, look_back_window, max_holding_number)
    ...
    def reset(self):
        single_state, trend_state, clf_state, minute1_state, minute3_state, minute5_state, done, info = super(Training_Env, self).reset()
        self.previous_action = self.initial_action
        self.previous_position = self.initial_action * self.max_holding_number
        self.position = self.initial_action * self.max_holding_number
        info['q_value'] = self.q_table[self.m - 1][self.previous_action][:]
        return single_state, trend_state, clf_state.reshape(-1), minute1_state, minute3_state, minute5_state, done, info

    def step(self, action):
        single_state, trend_state, clf_state, minute1_state, minute3_state, minute5_state, reward, done, info = super(Training_Env, self).step(action)
        info['q_value'] = self.q_table[self.m - 1][action][:]
        return single_state, trend_state, clf_state.reshape(-1), minute1_state, minute3_state, minute5_state, reward, done, info
