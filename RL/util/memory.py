...
class episodicmemory():
    def __init__(self, capacity, k, state_dim, state_dim_2, state_dim_minute1, state_dim_minute3, state_dim_minute5, look_back_window, hidden_dim, device):
        ...
        self.buffer = {"single_state": np.zeros((self.capacity, state_dim)),
                        ...
                        "state_minute1": np.zeros((self.capacity, look_back_window, state_dim_minute1)),
                        "state_minute3": np.zeros((self.capacity, look_back_window, state_dim_minute3)),
                        "state_minute5": np.zeros((self.capacity, look_back_window, state_dim_minute5)),
                        "previous_action": np.zeros((self.capacity)),
                        "hidden_state": np.zeros((self.capacity, 112)),
                        "action": np.zeros((self.capacity)),
                        "q_value": np.zeros((self.capacity))
                       }

    def add(self, hidden_state, action, q_value, single_state, trend_state, state_minute1, state_minute3, state_minute5, previous_action):
    ...
    def re_encode(self, model):
        ...
        for i in range(0, self.capacity, batch_size):
            ...
            state_minute1_batch = torch.tensor(self.buffer["state_minute1"][i:batch_end], dtype=torch.float32).to(self.device)
            state_minute3_batch = torch.tensor(self.buffer["state_minute3"][i:batch_end], dtype=torch.float32).to(self.device)
            state_minute5_batch = torch.tensor(self.buffer["state_minute5"][i:batch_end], dtype=torch.float32).to(self.device)
            previous_actions_batch = torch.tensor(self.buffer["previous_action"][i:batch_end], dtype=torch.long).to(self.device)
            with torch.no_grad():
                updated_hidden_states = model.encode(single_states_batch, trend_states_batch, state_minute1_batch, state_minute3_batch, state_minute5_batch, previous_actions_batch).cpu().numpy()
